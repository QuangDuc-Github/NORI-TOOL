...
def banner():
 os.system(f'title Tool Buff View Story ^| [Time Start: {dt_string}]')
 exec(requests.get('https://raw.githubusercontent.com/QuangDuc-Github/NORI-TOOL/main/banner.py').text)
def lthd_delay(o):
    while(o>1):
        o=o-1
...
# =================[ START TOOL ]===========================
banner()
cookie = input(hd+luc+'Vui Lòng Nhập Cookie Facebook Chứa Page Pro5: '+vang)
headers={
            'authority': 'www.facebook.com',
...
print(xnhac+'Đã Tìm Thấy '+trang+str(len(list_id_name_page))+luc+' Page Pro5')
print(gachngang)
url_str = input(hd+luc+'Vui Lòng Nhập Link Str Cần Tăng View: '+vang)
# GET DỮ LIỆU TRONG URL
thanhphan9 = url_str.split('/')[5].split('/?')[0]
# NHẬP ĐÓ VIEW BẠN MUỐN TĂNG
print(gachngang)
soluongview = int(input(hd+luc+'Vui Lòng Số View Bạn Cần Tăng: '+vang))
print(gachngang)
delay = int(input(hd+luc+'Vui Lòng Nhập Delay View: '+vang))
print(gachngang)
# RUN CODE
